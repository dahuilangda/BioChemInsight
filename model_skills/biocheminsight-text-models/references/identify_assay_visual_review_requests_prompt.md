...
4) 普通数值列、单位明确且同列值域一致的测量列、NA/ND/空值等，不应仅因短文本而复读。
...
- `ocr_value`: 当前抽取对象的 `value`
- `unit`: 当前抽取对象的 `unit`，未知则空字符串
- `method`: 当前抽取对象的 `method`，未知则空字符串
- `description`: 当前抽取对象的 `description`，未知则空字符串
