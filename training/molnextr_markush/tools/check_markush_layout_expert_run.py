...
def resolve_report_path(raw_path: Any, *, run_dir: Path) -> Path | None:
    ...
    if not path.is_absolute():
        ...
        try:
            ...
            pass
        for marker in ["training/molnextr_markush/runs/markush_sidecar_smoke"]:
            try:
                archived = Path("training/molnextr_markush/runs/legacy_runs/markush_sidecar_smoke") / path.relative_to(marker)
                if archived.exists():
                    return archived
            except ValueError:
                continue
...
def audit_split_manifest(metrics: dict[str, Any], split_manifest_path: str) -> list[str]:
    ...
    projection_manifest = load_projection_manifest(config.get("input_projection_manifest"))

    comparisons = [
        ("markush_label_csv", "markush_train_csv", observed.get("markush_label_csv")),
        ("eval_markush_label_csv", "markush_calibration_csv", observed.get("eval_markush_label_csv")),
    ]
    policy = manifest.get("policy") if isinstance(manifest.get("policy"), dict) else {}
    markush_only_positive_split = (
        manifest.get("schema_version") == "markush_only_split_manifest_v1"
        and policy.get("positive_only_markush_layout_training") is True
        and policy.get("router_negative_evidence_not_claimed") is True
    )
    if not markush_only_positive_split:
        negative_hashes = observed.get("negative_csv") if isinstance(observed.get("negative_csv"), list) else []
        eval_negative_hashes = observed.get("eval_negative_csv") if isinstance(observed.get("eval_negative_csv"), list) else []
        comparisons.extend(
            [
                ("negative_csv[0]", "ordinary_train_csv", negative_hashes[0] if negative_hashes else None),
                (
                    "eval_negative_csv[0]",
                    "ordinary_calibration_csv",
                    eval_negative_hashes[0] if eval_negative_hashes else None,
                ),
            ]
        )
    for observed_name, manifest_name, observed_hash in comparisons:
        ...
        elif str(observed_hash) != str(expected_hash) and not projection_matches(
            projection_manifest,
            observed_name=observed_name,
            manifest_name=manifest_name,
            observed_hash=str(observed_hash),
            expected_parent_hash=str(expected_hash),
        ):
            blockers.append(f"{observed_name} hash does not match current split manifest {manifest_name}")
    return blockers


def load_projection_manifest(path_text: Any) -> dict[str, Any]:
    if not path_text:
        return {}
...
def projection_matches(
    ...
    *,
    ...
    if not projection_manifest:
        return False
    if projection_manifest.get("schema_version") != "markush_training_label_projection_manifest_v1":
        return False
    policy = projection_manifest.get("policy") if isinstance(projection_manifest.get("policy"), dict) else {}
    required_policy = [
        "projection_preserves_row_count",
        "projection_preserves_image_paths",
        "projection_preserves_smiles_and_source_identity",
        "projection_preserves_markush_layout_cells",
        "projection_does_not_modify_images_coordinates_or_labels",
        "parent_split_hashes_must_match_current_manifest",
    ]
    if any(policy.get(key) is not True for key in required_policy):
        return False
    projections = projection_manifest.get("projections")
    if not isinstance(projections, dict):
        return False
    projection = projections.get(manifest_name)
    if not isinstance(projection, dict):
        return False
    observed_base = observed_name.split("[", 1)[0]
    if str(projection.get("training_arg")) != observed_base:
        return False
    return (
        str(projection.get("parent_sha256") or "") == str(expected_parent_hash)
        and str(projection.get("projection_sha256") or "") == str(observed_hash)
        and int(projection.get("row_count") or 0) > 0
        and projection.get("passed") is True
    )


def audit_confidence_report(path: Path, *, require_deployable: bool) -> tuple[list[str], list[str]]:
    blockers: list[str] = []
    warnings: list[str] = []
    if not path.exists():
        return [f"missing Markush confidence report: {path}"], warnings
    report = load_json(path)
    confidence = report.get("confidence") if isinstance(report.get("confidence"), dict) else {}
    for head in ["presence_confidence", "count_confidence", "risk_confidence", "sidecar_confidence"]:
    ...
    if require_deployable:
        ...
        hard_min_positive_recall = max(0.10, float(constraints.get("min_positive_layout_recall") or 0.0))
        ...
        if not isinstance(selected, dict):
            blockers.append("Markush run has no selected calibrated threshold")
        else:
            if int(selected.get("negative_accepted") or 0) > max_negative_accepted:
                blockers.append("selected Markush threshold exceeds negative accepted limit")
            if float(selected.get("positive_layout_precision") or 0.0) < float(
                constraints.get("min_positive_layout_precision") or 0.95
            ):
                blockers.append("selected Markush threshold does not meet layout precision")
            if int(selected.get("positive_accepted") or 0) < hard_min_positive_accepted:
                blockers.append(
                    f"selected Markush threshold accepts too few positives for formal confidence "
            ...
            if float(selected.get("coverage") or 0.0) < hard_min_coverage:
                blockers.append(
                    f"selected Markush threshold coverage is too low for formal confidence "
                    f"({float(selected.get('coverage') or 0.0):.6f} < {hard_min_coverage:.6f})"
                )
            if float(selected.get("positive_layout_recall") or 0.0) < hard_min_positive_recall:
                blockers.append(
                    f"selected Markush threshold positive layout recall is too low for formal confidence "
                    f"({float(selected.get('positive_layout_recall') or 0.0):.6f} < {hard_min_positive_recall:.6f})"
                )
            if "positive_layout_precision_wilson_lower" not in selected:
                blockers.append("selected Markush threshold is missing Wilson precision lower bound")
            elif float(selected.get("positive_layout_precision_wilson_lower") or 0.0) < hard_min_wilson:
                blockers.append("selected Markush threshold does not meet Wilson precision lower bound")
            if float(sidecar_confidence.get("ece") or 1.0) > hard_max_sidecar_ece:
                blockers.append(
                    f"Markush sidecar confidence ECE is too high for formal confidence "
            ...
            if float(sidecar_confidence.get("brier") or 1.0) > hard_max_sidecar_brier:
                blockers.append(
                    f"Markush sidecar confidence Brier is too high for formal confidence "
        ...
        if isinstance(selected_errors, dict):
            if int(selected_errors.get("negative_accepted_errors") or 0) > max_negative_accepted:
                blockers.append("selected Markush threshold has deployment-blocking ordinary-negative accepted errors")
        elif selected is not None and report.get("selected_high_confidence_errors") not in (0, 0.0):
            blockers.append("legacy Markush confidence report has selected-threshold high-confidence errors")
        if report.get("deployment_allowed") is not True:
            blockers.append("Markush confidence report does not allow deployment")
    elif report.get("deployment_allowed") is not True:
        warnings.append("Markush confidence report is present but not deployable")
...
def audit_model_scale(metrics: dict[str, Any], *, require_evidence_scale: bool) -> tuple[list[str], list[str]]:
    ...
    required_expert_keys = ["hidden_dim", "fusion_transformer_depth", "fusion_transformer_heads", "layout_queries"]
    ...
    if not str(expert_branch.get("sidecar_risk_target") or "").strip():
        blockers.append("metrics.model_scale.expert_branch.sidecar_risk_target is missing")
    elif str(expert_branch.get("sidecar_risk_target")) != "sidecar_emission_quality":
        warnings.append(
            "Markush sidecar_risk_target is not sidecar_emission_quality; run must be treated as an ablation unless justified"
        )
    negative_count_zero_loss_weight = expert_branch.get("negative_count_zero_loss_weight")
    if negative_count_zero_loss_weight in (None, ""):
        blockers.append("metrics.model_scale.expert_branch.negative_count_zero_loss_weight is missing")
    elif require_evidence_scale and float(negative_count_zero_loss_weight) <= 0.0:
        blockers.append("Markush measured/formal evidence requires explicit ordinary-negative count-zero supervision")
    ...
    observed_queries = int(scale_observed.get("layout_queries") or expert_branch.get("layout_queries") or 0)
    observed_patch = int(scale_observed.get("patch_size") or expert_branch.get("patch_size") or 0)
    if observed_hidden < 256:
        blockers.append(f"Markush expert hidden_dim {observed_hidden} < 256")
    if observed_depth < 4:
        blockers.append(f"Markush expert fusion depth {observed_depth} < 4")
    if observed_heads < 8:
        blockers.append(f"Markush expert attention heads {observed_heads} < 8")
    if observed_queries < 16:
        blockers.append(f"Markush layout queries {observed_queries} < 16")
    if observed_patch <= 0 or observed_patch > 16:
        blockers.append(f"Markush patch_size {observed_patch} is not in the formal range <=16")
    ...
    if routing.get("expert_branch") != "markush_layout":
        blockers.append("metrics.model_scale.routing.expert_branch is not markush_layout")
    ...
    if require_evidence_scale:
        if int(training_scale.get("gpu_count_observed") or 0) < 2:
            blockers.append("Markush measured/formal evidence requires two observed GPUs or a measured single-GPU rationale")
        if int(training_scale.get("world_size") or 0) < 2 and not str(training_scale.get("single_gpu_rationale") or "").strip():
            blockers.append("Markush measured/formal evidence lacks DDP world_size>=2 and has no measured single-GPU rationale")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate a Markush layout expert run against the frozen-base contract.")
    parser.add_argument("--run-dir", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--split-manifest", default="")
    parser.add_argument(
        "--allow-measured-acceptance-report",
        action="store_true",
        help=(
            "Allow a strict Markush measured acceptance report to satisfy measured-stage gating when no full "
            "training readiness report is available. This does not apply to formal-stage validation."
        ),
    )
    ...
    checkpoint_path = run_dir / "markush_layout_expert.pth"
    blockers: list[str] = []
    warnings: list[str] = []
    ...
    if not checkpoint_path.exists():
        blockers.append(f"missing Markush layout expert checkpoint: {checkpoint_path}")

    if metrics:
        if metrics.get("training_type") != "frozen_molnextr_encoder_markush_layout_expert":
            blockers.append("metrics.training_type is not Markush layout expert")
        ...
        if metrics.get("consumes_markush_split") is not True:
            blockers.append("run did not declare consumes_markush_split=true")
        ...
        if args.require_measured_stage or args.require_formal_stage:
            ...
            training_controls = (
                metrics.get("training_controls") if isinstance(metrics.get("training_controls"), dict) else {}
            )
            if "negative_count_zero_loss_weight" not in training_controls:
                blockers.append("metrics.training_controls.negative_count_zero_loss_weight is missing")
            elif float(training_controls.get("negative_count_zero_loss_weight") or 0.0) <= 0.0:
                blockers.append("measured/formal Markush run did not enable ordinary-negative count-zero loss")
            count_prediction_source = str(training_controls.get("count_prediction_source") or "")
            if (
                count_prediction_source in {"corn", "hybrid"}
                and float(training_controls.get("count_corn_loss_weight") or 0.0) <= 0.0
            ):
                blockers.append("measured/formal Markush run used corn/hybrid count source without trained CORN loss")
        gate = metrics.get("gate_report") if isinstance(metrics.get("gate_report"), dict) else {}
        if gate.get("enforced") is not True or gate.get("accepted") is not True:
            blockers.append("gate_report is not enforced and accepted")
        ...
        if args.require_measured_stage:
            ...
            measured_acceptance_gate = (
                args.allow_measured_acceptance_report
                and gate.get("accepted") is True
                and gate.get("measured_sidecar_smoke_allowed") is True
                and gate.get("required_key") == "measured_sidecar_smoke_allowed"
                and gate.get("formal_training_allowed") is False
            )
            if not measured_acceptance_gate:
        ...
        if args.require_formal_stage and gate.get("training_stage") != "formal":
            blockers.append("formal validation requires gate_report.training_stage=formal")
        ...
        if not epochs:
            ...
            if not isinstance(eval_report, dict):
                blockers.append("last epoch has no eval report")
            else:
                if float(eval_report.get("presence_accuracy") or 0.0) <= 0.0:
                    warnings.append("Markush presence accuracy is zero")
                if float(eval_report.get("positive_count_accuracy") or 0.0) <= 0.0:
                    warnings.append("Markush variable-count accuracy is zero")
                if float(eval_report.get("layout_box_l1") or 1.0) >= 0.5:
                    warnings.append("Markush layout box L1 is high")
            ...
            if require_deployable_confidence:
                if confidence_report_path is None:
                    blockers.append("Markush run missing epoch confidence_report path required for calibrated confidence")
                else:
                    confidence_blockers, confidence_warnings = audit_confidence_report(
    ...
    report = {
        "schema_version": "markush_layout_expert_run_contract_v2",
        "run_dir": str(run_dir),
        ...
        "policy": {
            ...
            "markush_split_must_be_consumed": True,
            ...
            "measured_stage_acceptance_report_fallback_allowed": bool(args.allow_measured_acceptance_report),
